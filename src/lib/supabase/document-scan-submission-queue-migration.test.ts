...
const failureSql = readFileSync(resolve("supabase/migrations/20260818143000_document_scan_non_consuming_failures.sql"), "utf8").toLowerCase();
describe("document scan submission queue", () => {
  ...
  it("preserves retry budget for infrastructure identity failures", () => {
    expect(failureSql).toContain("record_document_scan_submission_failure_v2");
    expect(failureSql).toContain("p_consumes_attempt");
    expect(failureSql).toContain("greatest(v_job.attempt_count - 1, 0)");
    expect(failureSql).toContain("service_role");
    expect(failureSql).toContain("revoke all");
    expect(failureSql).toContain("grant execute");
  });
});
