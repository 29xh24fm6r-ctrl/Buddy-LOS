...
export default async function DealPage({ params, searchParams }: { params: Promise<{ dealId: string }>; searchParams: Promise<{ underwriting?: string; error?: string; saved?: string }> }) {
  ...
  return <AppShell context={context} surface={defaultWorkspaceSurface(context.workspace)}><DealCockpit deal={deal} downloadsEnabled={downloadsEnabled} uploadsEnabled={uploadsEnabled} scanRecoveryEnabled={scanRecoveryEnabled} underwriting={underwriting} runtimeEnabled={process.env.BUDDY_UNDERWRITER_RUNTIME_ENABLED === "true"} underwritingOutcome={query.underwriting} role={context.activeOrganization.role} lifecycle={lifecycle} factoryEnabled={factoryEnabled} /><CoreDealOperations dealId={deal.id} version={deal.version} stage={deal.stage} tasks={tasks} writesEnabled={writesEnabledForOrganization(context.activeOrganization.organizationId)} error={query.error} saved={query.saved}/></AppShell>;
}
