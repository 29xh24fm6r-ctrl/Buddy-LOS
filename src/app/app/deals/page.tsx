...
export default async function DealsPage({ searchParams }: { searchParams: Promise<{ q?: string; stage?: string; surface?: string; view?: string }> }) {
  ...
  const { q = "", stage = "", surface: requestedSurface, view = "active" } = await searchParams;
  ...
  return <AppShell context={context} surface={surface}><AppHeader context={context} eyebrow="Deal Workspace" title={view === "active" ? "Active pipeline" : "Operating work queue"} pipelineAmount={deals.reduce((sum, deal) => sum + (deal.approvedAmount ?? deal.requestedAmount ?? 0), 0)} activeDeals={deals.length} />{readsEnabled ? <DealPipeline deals={deals} query={q} stage={stage} view={view} surface={surface} /> : <ReadsPending />}</AppShell>;
}
