...
import { activityKinds, borrowerKinds, canOperateCore, contactKinds, dateTimeValue, decimalValue, enumValue, optionalText, relationshipKinds, requiredText, uuidValue, validContactValue } from "@/lib/los/core-operations";
...
export async function updateCrmTask(form:FormData){
  const context=await commandContext(),taskId=uuidValue(form,"taskId"),idempotencyKey=requiredText(form,"idempotencyKey",8,160),title=requiredText(form,"title",2,200),description=optionalText(form,"description",4000),dueAt=dateTimeValue(form,"dueAt",true),assignedTo=uuidValue(form,"assignedTo",true),expectedVersion=Number(form.get("expectedVersion")),cancel=form.get("cancel")==="true";
...
export async function createCrmReferral(form:FormData){
  const context=await commandContext(),borrowerId=uuidValue(form,"borrowerId"),sourceBorrowerId=uuidValue(form,"sourceBorrowerId"),dealId=uuidValue(form,"dealId",true),idempotencyKey=requiredText(form,"idempotencyKey",8,160),referredAt=dateTimeValue(form,"referredAt"),estimatedValue=decimalValue(form,"estimatedValue",true),notes=optionalText(form,"notes",4000);
  if(!borrowerId||!sourceBorrowerId||borrowerId===sourceBorrowerId||dealId===undefined||!idempotencyKey||!referredAt||estimatedValue===undefined||(estimatedValue!==null&&estimatedValue<0)||notes===null)redirect("/app/crm?view=referrals&error=invalid-input");
  const supabase=await createClient();const{error}=await supabase.rpc("create_crm_referral",{p_organization_id:context.activeOrganization.organizationId,p_borrower_id:borrowerId,p_source_borrower_id:sourceBorrowerId,p_source_person_id:null,p_deal_id:dealId,p_idempotency_key:idempotencyKey,p_referred_at:referredAt,p_estimated_value:estimatedValue,p_notes:notes});
...
export async function createCrmAppointment(form:FormData){
  const context=await commandContext(),borrowerId=uuidValue(form,"borrowerId"),dealId=uuidValue(form,"dealId",true),idempotencyKey=requiredText(form,"idempotencyKey",8,160),subject=requiredText(form,"subject",2,200),startsAt=dateTimeValue(form,"startsAt"),endsAt=dateTimeValue(form,"endsAt"),location=optionalText(form,"location",500),notes=optionalText(form,"notes",4000);
