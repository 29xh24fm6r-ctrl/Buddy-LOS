...
export async function createCrmCompany(form: FormData) {
  ...
  if(error||typeof borrowerId!=="string") commandFailed(context,"create_crm_company",error??{message:"CRM company command returned no borrower id."},"/app/crm?view=companies&error=command-failed");
...
export async function createCrmContact(form: FormData) {
  ...
  if(error) commandFailed(context,"create_crm_contact",error,"/app/crm?view=people&error=command-failed"); revalidatePath("/app/crm"); redirect(`/app/borrowers/${borrowerId}`);
}
...
export async function createCrmRelationship(form: FormData) {
  ...
  if(error) commandFailed(context,"create_crm_relationship",error,"/app/crm?view=relationships&error=command-failed"); revalidatePath("/app/crm"); redirect("/app/crm?view=relationships&saved=relationship");
}
...
export async function createCrmTask(form: FormData) {
  ...
  if(error) commandFailed(context,"create_deal_task",error,"/app/crm?view=tasks&error=command-failed"); revalidatePath("/app/crm"); redirect("/app/crm?view=tasks&saved=task");
}
...
export async function completeCrmTask(form: FormData) {
  ...
  if(error) commandFailed(context,"complete_deal_task",error,`/app/crm?view=tasks&error=${error.code==="40001"?"version-conflict":"command-failed"}`); revalidatePath("/app/crm"); redirect("/app/crm?view=tasks&saved=task");
}
...
export async function logCrmActivity(form: FormData) {
  ...
  if(error) commandFailed(context,"log_crm_activity",error,"/app/crm?view=activities&error=command-failed"); revalidatePath("/app/crm"); redirect("/app/crm?view=activities&saved=true");
}
...
export async function createCrmPerson(form:FormData){
  ...
  if(error)commandFailed(context,"create_crm_person",error,"/app/crm?view=people&error=command-failed");revalidatePath("/app/crm");redirect("/app/crm?view=people&saved=person");
}
...
export async function updateCrmCompany(form:FormData){
  ...
  if(error)commandFailed(context,"update_crm_company",error,`/app/crm?view=companies&error=${error.code==="40001"?"version-conflict":"command-failed"}`);revalidatePath("/app/crm");redirect("/app/crm?view=companies&saved=company");
}
...
export async function archiveCrmContact(form:FormData){
  ...
  if(error)commandFailed(context,"archive_crm_contact",error,`/app/crm?view=people&error=${error.code==="40001"?"version-conflict":"command-failed"}`);revalidatePath("/app/crm");redirect("/app/crm?view=people&saved=contact");
}
...
export async function closeCrmRelationship(form:FormData){
  ...
  if(error)commandFailed(context,"close_crm_relationship",error,`/app/crm?view=relationships&error=${error.code==="40001"?"version-conflict":"command-failed"}`);revalidatePath("/app/crm");redirect("/app/crm?view=relationships&saved=relationship");
}
...
export async function updateCrmTask(form:FormData){
  ...
  if(error)commandFailed(context,"update_deal_task",error,`/app/crm?view=tasks&error=${error.code==="40001"?"version-conflict":"command-failed"}`);revalidatePath("/app/crm");redirect("/app/crm?view=tasks&saved=task");
}
...
export async function createCrmReferral(form:FormData){
  ...
  if(error)commandFailed(context,"create_crm_referral",error,"/app/crm?view=referrals&error=command-failed");revalidatePath("/app/crm");redirect("/app/crm?view=referrals&saved=referral");
}
...
export async function createCrmAppointment(form:FormData){
  ...
  if(error)commandFailed(context,"create_crm_appointment",error,"/app/crm?view=calendar&error=command-failed");revalidatePath("/app/crm");redirect("/app/crm?view=calendar&saved=appointment");
}
...
export async function reopenCrmRelationship(form:FormData){
  const context=await commandContext(),relationshipId=uuidValue(form,"relationshipId"),idempotencyKey=requiredText(form,"idempotencyKey",8,160);
  if(!relationshipId||!idempotencyKey)redirect("/app/crm?view=relationships&error=invalid-input");
  const supabase=await createClient();const{error}=await supabase.rpc("reopen_crm_relationship",{p_organization_id:context.activeOrganization.organizationId,p_relationship_id:relationshipId,p_idempotency_key:idempotencyKey,p_expected_active:false});
  if(error)commandFailed(context,"reopen_crm_relationship",error,`/app/crm?view=relationships&error=${error.code==="40001"?"version-conflict":"command-failed"}`);revalidatePath("/app/crm");redirect("/app/crm?view=relationships&saved=relationship");
}

export async function restoreCrmRecord(form:FormData){
  const context=await commandContext(),entityId=uuidValue(form,"entityId"),entity=String(form.get("entity")??""),idempotencyKey=requiredText(form,"idempotencyKey",8,160),expectedVersion=Number(form.get("expectedVersion"));
  if(!entityId||!["company","contact","person"].includes(entity)||!idempotencyKey||!Number.isInteger(expectedVersion)||expectedVersion<1)redirect("/app/crm?error=invalid-input");
  const view=entity==="company"?"companies":"people",supabase=await createClient();const{error}=await supabase.rpc("restore_crm_record",{p_organization_id:context.activeOrganization.organizationId,p_entity:entity,p_entity_id:entityId,p_idempotency_key:idempotencyKey,p_expected_version:expectedVersion});
  if(error)commandFailed(context,"restore_crm_record",error,`/app/crm?view=${view}&error=${error.code==="40001"?"version-conflict":"command-failed"}`);revalidatePath("/app/crm");redirect(`/app/crm?view=${view}&saved=restored`);
}
