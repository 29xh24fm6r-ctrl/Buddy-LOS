...
export default async function CrmPage({ searchParams }: { searchParams: Promise<{ q?: string; view?: string }> }) {
  ...
  const { q = "", view = "home" } = await searchParams;
  ...
  return <AppShell context={context} surface="crm">{readsEnabled ? <BorrowerDirectory rows={rows} query={q} view={view} /> : <ReadsPending />}</AppShell>;
}
