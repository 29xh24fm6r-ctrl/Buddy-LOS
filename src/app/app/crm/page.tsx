...
export default async function CrmPage({ searchParams }: { searchParams: Promise<{ q?: string; view?: string; error?: string; saved?: string }> }) {
  ...
  return <AppShell context={context} surface="crm">{readsEnabled ? <><BorrowerDirectory rows={rows} query={q} view={view} /><CrmOperationsPanel rows={rows} activities={activities} writesEnabled={readFoundationStatus().writesEnabled} error={error} saved={saved} /></> : <ReadsPending />}</AppShell>;
}
