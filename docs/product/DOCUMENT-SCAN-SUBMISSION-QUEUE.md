...
The worker is disabled unless `BUDDY_DOCUMENT_SCANNING_ENABLED=true`, requires Vercel `CRON_SECRET`, and requires complete HTTPS scanner configuration. No `vercel.json` schedule is registered in this slice: production cadence depends on the selected Vercel plan and approved scan-latency objective. The executable [provider contract](DOCUMENT-SCANNER-PROVIDER-CONTRACT.md) now proves the outbound request and response boundary; selected-vendor sandbox tests, rejected-object operations, live retry/recovery evidence, monitoring, and explicit activation approval remain required.
